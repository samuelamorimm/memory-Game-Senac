...
document.getElementById("start").addEventListener("click", function start(){
    ...
    for (let i = 0; i<16 ; i++) {
        fig[i].style.animation = 'girar 1s'
        fig[i].classList.add("virada")
    }

    setTimeout(() => {
        for (let i = 0; i<16 ; i++) {
            fig[i].style.animation = 'voltar 1s'
            fig[i].classList.remove("virada")
        }
    }, 2000);
    ...
    if (paresEncontrados < 8) {
        ...
        function atualizarCronometro() {
            ...
            segundos++;
            cronometro.textContent = `Segundos: ${segundos}`;
        }
